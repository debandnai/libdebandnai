...
class LinkagePager : ViewGroup {
    /**
     * Used to track what the expected number of items in the adapter should be.
     * If the app changes this when we don't expect it, we'll throw a big obnoxious exception.
     */
    private var mExpectedAdapterCount = 0
    private var linkagePager: LinkagePager? = null
    ...
    fun setLinkagePager(linkagePager: LinkagePager?) {
    ...
    override fun addView(child: View, index: Int, params: ViewGroup.LayoutParams) {
        ...
        val lp: LayoutParams? = params as LayoutParams
    ...
    private fun performDrag(x: Float): Boolean {
        ...
        val pageOffset = scrollX / width
        if (linkagePager != null) {
            linkagePager!!.scrollTo(
                (pageOffset * linkagePager!!.width).toInt(),
                linkagePager!!.scrollY
            )
        }
            ...
            for (pos in firstPos until lastPos) {
                ...
                if (drawAt + mPageMargin > scrollX) {
                    mMarginDrawable!!.setBounds(
                        drawAt.toInt(), mTopPageBounds,
                        (drawAt + mPageMargin + 0.5f) as Int, mBottomPageBounds
                    )
    ...
    internal inner class MyAccessibilityDelegate() : AccessibilityDelegateCompat() {
        override fun onInitializeAccessibilityEvent(host: View, event: AccessibilityEvent) {
            ...
            event.className = LinkagePager::class.java.name
        ...
        override fun onInitializeAccessibilityNodeInfo(
            ...
            info.className = LinkagePager::class.java.name
    ...
    companion object {
        ...
        private var COMPARATOR: Comparator<ItemInfo> =

            object : Comparator<ItemInfo> {
                override fun compare(lhs: ItemInfo?, rhs: ItemInfo?): Int {
                    return rhs?.position?.let { (lhs?.position)?.minus(it) }!!
                }
            }
