...
class SlideMenuDrawerLayout @JvmOverloads constructor(
    context: Context?,
    ...
    private var mDrawerListener: DrawerLayout.DrawerListener? = null
    ...
    private fun readAttributes(attributeSet: AttributeSet?) {
        val typedArray =
            context.obtainStyledAttributes(attributeSet, R.styleable.SlideMenuDrawerLayout)
        try {
            mMenuViewId = typedArray.getResourceId(
                R.styleable.SlideMenuDrawerLayout_menu,
                DEFAULT_ATTRIBUTE_VALUE
            )
            mContentViewId = typedArray.getResourceId(
                R.styleable.SlideMenuDrawerLayout_content,
                DEFAULT_ATTRIBUTE_VALUE
            )
            mContentScaleClosed = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_contentScaleClosed,
                CONTENT_SCALE_CLOSED
            )
            mContentScaleOpen = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_contentScaleOpen,
                CONTENT_SCALE_OPEN
            )
            mMenuScaleClosed = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_menuScaleClosed,
                MENU_SCALE_CLOSED
            )
            mMenuScaleOpen = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_menuScaleOpen,
                MENU_SCALE_OPEN
            )
            mMenuAlphaClosed = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_menuAlphaClosed,
                MENU_ALPHA_CLOSED
            )
            mMenuAlphaOpen = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_menuAlphaOpen,
                MENU_ALPHA_OPEN
            )
            mMarginFactor =
                typedArray.getFloat(R.styleable.SlideMenuDrawerLayout_marginFactor, MARGIN_FACTOR)
            mClickToCloseScale = typedArray.getFloat(
                R.styleable.SlideMenuDrawerLayout_clickToCloseScale,
    ...
    private fun showTouchInterceptor() {
        ...
        val offset = map(
            ...
            0f,
            this@SlideMenuDrawerLayout.width * mMarginFactor,
            0f,
            1f
        )
    ...
    private fun addTouchInterceptor() {
        val touchInterceptor = mLayoutInflater!!.inflate(R.layout.slide_menu_overlay, this, false)
        ...
        if (drawerWidth == 0) {
            viewTreeObserver.addOnPreDrawListener(object : ViewTreeObserver.OnPreDrawListener {
                override fun onPreDraw(): Boolean {
                    this@SlideMenuDrawerLayout.viewTreeObserver.removeOnPreDrawListener(this)
                    if (mViewDragHelper!!.smoothSlideViewTo(
                        ...
                        ViewCompat.postInvalidateOnAnimation(this@SlideMenuDrawerLayout)
                    }
                    return false
                }
            })
     ...
     * @see DrawerListener
     */
    fun setDrawerListener(drawerListener: DrawerLayout.DrawerListener?) {
    ...
    private inner class ViewDragCallback() : ViewDragHelper.Callback() {
        ...
        override fun getViewHorizontalDragRange(child: View): Int {
            return this@SlideMenuDrawerLayout.measuredWidth
        }
        ...
        override fun onViewPositionChanged(
            ...
            mDragOffset =
                map(left.toFloat(), 0f, this@SlideMenuDrawerLayout.width * mMarginFactor, 0f, 1f)
            ...
            if (mDrawerListener != null) {
                mDrawerListener!!.onDrawerSlide(this@SlideMenuDrawerLayout, mDragOffset)
            }
        }
        ...
        override fun onViewDragStateChanged(state: Int) {
            ...
            if (state == STATE_IDLE) {
                if (mDragOffset == 0f) {
                    ...
                    setViewAndChildrenEnabled(mMenuView, false)
                    if (mDrawerListener != null) {
                        mDrawerListener!!.onDrawerClosed(this@SlideMenuDrawerLayout)
                    }
                } else if (mDragOffset == 1f) {
                    showTouchInterceptor()
                    setViewAndChildrenEnabled(mMenuView, true)
                    if (mDrawerListener != null) {
                        mDrawerListener!!.onDrawerOpened(this@SlideMenuDrawerLayout)
                    }
                }
            }
    ...
    companion object {
        ...
        private val CONTENT_SCALE_OPEN = 0.8f //actual scale
        private val CLICK_TO_CLOSE_SCALE = 0.8f // click area
        ...
        private val MARGIN_FACTOR = 0.6f // left margin
